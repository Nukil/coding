...
import com.netposa.rpc.HumanFeatureDataAnalyzeRpcService;
...
public class HumanFeatureDataAnalyzeRpcServer implements Runnable {
	private static final Logger LOG = LoggerFactory.getLogger(HumanFeatureDataAnalyzeRpcServer.class);
	public final int port;

	public HumanFeatureDataAnalyzeRpcServer(int port) {
		this.port = port;
	}
	/**
	 * 协议/客户端和服务端必须保持一致
	...
	@Override
	public void run() {
		final HumanFeatureDataAnalyzeRpcServiceHandler handler = new HumanFeatureDataAnalyzeRpcServiceHandler();
		final HumanFeatureDataAnalyzeRpcService.Processor<HumanFeatureDataAnalyzeRpcServiceHandler> p = new HumanFeatureDataAnalyzeRpcService.Processor<HumanFeatureDataAnalyzeRpcServiceHandler>(handler);
