...
class DirectKafkaCluster(val kafkaParams: Map[String, String]) extends Logging {
  import DirectKafkaCluster.{Err, LeaderOffset, SimpleConsumerConfig}
  
  val debug = kafkaParams.getOrElse("receive.process.debug.enable", "false").toBoolean
    ...
    getPartitionMetadata(topics).right.map { r =>
      r.flatMap { tm: TopicMetadata =>
        tm.partitionsMetadata.map { pm: PartitionMetadata =>
          if (debug) {
            info("find topic is %s and partition is %d".format(tm.topic,pm.partitionId))
          }
  ...
  def getPartitionMetadata(topics: Set[String]): Either[Err, Set[TopicMetadata]] = {
    ...
    withBrokers(Random.shuffle(config.seedBrokers), errs) { consumer =>
      ...
      val set = resp.topicsMetadata.toSet.filter(p => topics.contains(p.topic))
      if (debug) {
        info("find metadata is %s".format(set.toString))
      }
      return Right(set)
    }
    Left(errs)
  ...
  def setConsumerOffsetMetadata(
    ...
    val req = OffsetCommitRequest(groupId, metadata,0)
    val errs = new Err
...
object DirectKafkaCluster {
  ...
  class SimpleConsumerConfig private(brokers: String, originalProps: Properties)
    ...
    val seedBrokers: Array[(String, Int)] = brokers.split(",").map { hp =>
      val hpa = hp.split(":")
      if (hpa.size == 1) {
        throw new Exception(s"Broker not the in correct format of <host>:<port> [$brokers]")
      }
      (hpa(0), hpa(1).toInt)
    }
  }
