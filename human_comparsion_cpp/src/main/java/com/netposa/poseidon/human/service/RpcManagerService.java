...
public class RpcManagerService extends Thread {
            ...
            for (Map.Entry<ConnectionManagerKey, ConnectionManagerValue> entry : cluster.entrySet()) {
                for (int i = 0; i < rpcPoolSize; i++) {
                    ...
                    if (null == cs) {
                        ...
                        HumanFeatureDataAnalyzeRpcService.Client client = RpcUtil.getInRpcConnection(entry.getKey().getIp(), entry.getKey().getPort(), entry.getKey().getTimeout());
                        if (null != client) {
                            try {
                                client.ping();
    ...
    public AvailableConnectionBean getAvailableConnection(ConnectionManagerKey key, boolean flag) {
        ...
        if (cluster.containsKey(key)) {
            ...
            // 尝试创建一个连接
            HumanFeatureDataAnalyzeRpcService.Client client = RpcUtil.getInRpcConnection(key.getIp(), key.getPort(), key.getTimeout());
            if (null != client) {
                try {
                    client.ping();
