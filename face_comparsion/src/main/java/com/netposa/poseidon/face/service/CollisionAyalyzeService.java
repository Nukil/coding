...
import com.netposa.poseidon.face.rpc.RegionRecord;
...
public class CollisionAyalyzeService {
	private static final String tableName = LoadPropers.getProperties().getProperty("face.table.name", "face_feature").trim();// 表名
	private static final Logger log = LoggerFactory.getLogger(CollisionAyalyzeService.class);
	private static String cameraIdType = LoadPropers.getProperties().getProperty("camera.id.type", "long");
