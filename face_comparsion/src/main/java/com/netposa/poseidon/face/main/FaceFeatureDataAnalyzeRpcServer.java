...
import com.netposa.poseidon.face.rpc.FaceFeatureDataAnalyzeRpcService;
...
public class FaceFeatureDataAnalyzeRpcServer implements Runnable {
	private static final Logger LOG = LoggerFactory.getLogger(FaceFeatureDataAnalyzeRpcServer.class);
	public final int port;

	public FaceFeatureDataAnalyzeRpcServer(int port) {
		this.port = port;
	}
	/**
	 * 协议/客户端和服务端必须保持一致
	...
	@Override
	public void run() {
		final FaceFeatureDataAnalyzeRpcServiceHandler handler = new FaceFeatureDataAnalyzeRpcServiceHandler();
		final FaceFeatureDataAnalyzeRpcService.Processor<FaceFeatureDataAnalyzeRpcServiceHandler> p = new FaceFeatureDataAnalyzeRpcService.Processor<FaceFeatureDataAnalyzeRpcServiceHandler>(handler);
