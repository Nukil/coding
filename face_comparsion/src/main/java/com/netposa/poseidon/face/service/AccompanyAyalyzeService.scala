...
import java.util
import java.util.{ArrayList, Calendar, Collections, PriorityQueue}
...
import com.netposa.poseidon.face.rpc.{AccompanyAyalyzeInputRecord, AccompanyAyalyzeResult, SearchImgByImgInputRecord}
import com.netposa.poseidon.face.util.{FaceFeatureVerify, HbaseUtil}
...
object AccompanyAyalyzeService extends Logging {
  ...
  val TABLE_NAME: String = LoadPropers.getProperties().getProperty("face.table.name", "face_feature").trim()
  val DEBUG: Boolean = LoadPropers.getProperties().getProperty("service.process.debug.enable", "true").toBoolean
  val cameraIdType: String = LoadPropers.getProperties().getProperty("camera.id.type", "long")
  //去重阈值
  val distinct: Int = LoadPropers.getProperties().getProperty("accompany.face.distinct", "30").trim().toInt * 1000
  ...
  def executeQuery(record: AccompanyAyalyzeInputRecord, pools: ExecutorService): java.util.List[AccompanyAyalyzeResult] = {
    ...
    val dates: util.ArrayList[String] = new util.ArrayList[String]
    ...
    //以图搜图
    val faceImgList = SearchFaceImgByImgService.executeQuery(searchFaceImgByImgInputRecord, dates)
    //排序
