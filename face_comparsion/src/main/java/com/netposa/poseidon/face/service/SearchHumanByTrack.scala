...
import com.netposa.poseidon.face.rpc.SearchHumanByTrackInputRecord
...
object SearchHumanByTrack extends Logging {
    val cameraIdType: String = LoadPropers.getProperties().getProperty("camera.id.type", "long")
    val tableName: String = LoadPropers.getProperties().getProperty("face.table.name", "face_feature")
    var CONN: Connection = _

    def executeQuery(record: SearchHumanByTrackInputRecord, pools: ExecutorService): java.util.ArrayList[java.util.ArrayList[String]] = {
