...
import com.netposa.poseidon.face.bean.FetcherDataBean;
...
public class ParseBytesUtil {
     ...
     * 数据组装
     */
    FaceFeature dataAssembleToPB(byte[] msg, int featureSize) {
        FaceFeature remsg = new FaceFeature();
        try {
    ...
    public FaceFeature dataForETL2PB(byte[] rowmessage, int featureSize) {
        if (rowmessage != null) {
            //数据组装
