...
import com.netposa.poseidon.face.rpc.inrpc.RecentDataBean;
import com.netposa.poseidon.face.rpc.inrpc.SetRecentDataInputRecord;
import com.netposa.poseidon.face.rpc.inrpc.SetRecentDataResponse;
import com.netposa.poseidon.face.rpc.inrpc.StatusCode;
import com.netposa.poseidon.face.util.Byte2FloatUtil;
import com.netposa.poseidon.face.util.HashAlgorithm;
import com.netposa.poseidon.face.util.ParseBytesUtil;
...
public class FetchMessageService extends Thread {
    ...
    public FetchMessageService(Properties serverProperties, Properties kafkaProperties) {
        ...
        this.featureVersionSize = Integer.parseInt(serverProperties.getProperty("face.version.size"));
        this.featureSize = Integer.parseInt(serverProperties.getProperty("face.feature.size"));
        this.featureTailSize = Integer.parseInt(serverProperties.getProperty("face.tail.size"));
        this.fullFeatureSize = featureVersionSize + featureSize + featureTailSize;
        this.featRawDim = featureSize / floatSize;
        this.arithmeticType = serverProperties.getProperty("arithmetic.type");
        // 设置算法版本校验位
        if (null != this.arithmeticType && "netposa".equals(this.arithmeticType)) {
            this.enableCheck = true;
            this.arithmeticVersion = serverProperties.getProperty("face.arithmetic.version");
            if ("1.3".equals(this.arithmeticVersion)) {
                this.arithmeticLabel = new byte[]{1, 0, 3, 0};
            } else if ("1.5".equals(this.arithmeticVersion)) {
                this.arithmeticLabel = new byte[]{1, 0, 5, 0};
            }
        } else {
        ...
        while (!stopped) {
            try {
                ...
                logger.info(String.format("streaming data statistics [service_name:%s,total_input:%s,current_input:%s,total_process:%s,current_process:%s,total_output:%s,current_output:%s]",
                        "face_comparsion_master", kafkaTotalCount, messages.size(), totalprocessCount, messages.size(), serverTotalCount, outputDataCount));
                if (debug) {
    ...
    private void executeBatch(List<RowMessage> messages, int size) {
        ...
        for (RowMessage message : messages) {
            FaceFeature humanFeature = new FaceFeature();
            try {
            ...
            // 目前只有深网的人脸算法才开启算法版本校验
            if (this.enableCheck) {
                // 特征为空 跳过
                if (null == humanFeature.getFeature() || humanFeature.getFeature().length < featureVersionSize) {
                    continue;
                }
                byte[] featureVersion = new byte[4];
                System.arraycopy(humanFeature.getFeature(), 4, featureVersion, 0, 4);
                if (!Arrays.equals(this.arithmeticLabel, featureVersion)) {
                    continue;
                }
            }
     ...
     */
    private boolean writeData2Hbase(List<FaceFeature> list) {
        List<Put> puts = new ArrayList<>();
        if (list.isEmpty()) {
            logger.warn("data list is empty");
            return true;
        } else {
            for (FaceFeature humanFeature : list) {
