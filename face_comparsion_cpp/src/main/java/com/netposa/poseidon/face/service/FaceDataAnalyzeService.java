...
import com.netposa.poseidon.face.rpc.outrpc.FaceFeatureDataAnalyzeRpcService;
...
public class FaceDataAnalyzeService implements Runnable {

    private static final Logger LOG = LoggerFactory.getLogger(FaceDataAnalyzeService.class);
    public final int port;

    public FaceDataAnalyzeService(int port) {
        this.port = port;
    }
    /**
     * 协议/客户端和服务端必须保持一致
    ...
    @Override
    public void run() {
        final FaceDataAnalyzeServiceHandler handler = new FaceDataAnalyzeServiceHandler();
        final FaceFeatureDataAnalyzeRpcService.Processor<FaceDataAnalyzeServiceHandler> p = new FaceFeatureDataAnalyzeRpcService.Processor<FaceDataAnalyzeServiceHandler>(handler);
