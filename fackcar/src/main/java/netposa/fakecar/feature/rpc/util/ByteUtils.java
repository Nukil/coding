...
public final class ByteUtils {
   ...
   * @return
   */
  public static long parseBytesToLongtime(byte[] array,Map<DateFormatDefine, int[]> date_pattern) {
  ...
  /**
   * 通过指定的日期格式把按此格式进行存储的一个byte array数组的值中小时部分的值,转换成integer值
   * 
   * @param array
   * @param pattern
   * @return
   */
  public static int parseBytesHour(byte[] array,Map<DateFormatDefine, int[]> date_pattern) {
    int hour = -1;
    int[] arr = date_pattern.get(DateFormatDefine.HOUR);
    if (arr[0] != -1 && array.length >= (arr[0] + arr[1])) {
      hour = parseInt(array, arr[0], arr[1]);
    }
    return hour;
