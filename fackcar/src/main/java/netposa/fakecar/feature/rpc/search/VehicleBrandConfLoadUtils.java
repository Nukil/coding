...
/**
 * 加载classpath中的vehicle_brand_conf.xml配置文件,并生成SimilarVehicleTableDef实例
 * @author dl
 *
 */
public class VehicleBrandConfLoadUtils {
	
	private static final Logger LOG = LoggerFactory.getLogger(VehicleBrandConfLoadUtils.class);
	
	private static final String conf_name = "vehicle_brand_conf.xml";
	
	public static Map<String,String> loadConfigByClassPath() {
		Map<String,String> map = new HashMap<String, String>();
		InputStream inStream = null;
		try {
			inStream = VehicleBrandConfLoadUtils.class.getClassLoader().getResourceAsStream(conf_name);
			
			DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document doc = db.parse(inStream);
			NodeList list = doc.getElementsByTagName("VehicleBrand");
			if (list.getLength() < 1) {
				throw new IOException("input vehicle_brand_conf.xml not found VehicleColor element");
			}
