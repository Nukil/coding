...
import com.netposa.poseidon.library.rpc.outrpc.ImgScore;
import com.netposa.poseidon.library.rpc.outrpc.QueryRequest;
import com.netposa.poseidon.library.rpc.outrpc.RecordInfo;
...
public class SearchImgResultFromHbase {
    ...
    private final static int regionNum = Integer.parseInt(properties.getProperty("hbase.split.region.num", "50").trim());
    private final static int version = Integer.parseInt(properties.getProperty("face.version.size", "16").trim());
    public static PriorityQueue<RecordInfo> execute(QueryRequest request, PriorityQueue<RecordInfo> recordInfos) {
        PriorityQueue<RecordInfo>  responseInfo = new PriorityQueue<>(request.rCount, dataQueryComparator);
