namespace java com.netposa.poseidon.library.rpc.inrpc
enum StatusCode
{
  OK = 1,                      //操作成功
  ERROR_PARAM = 2,   	         //参数错误
...
service LibraryDataAnalyzeRpcService {
  //库操作接口
  LibraryOperationResponse libraryOperate(1:LibraryOperation request, 2:string uuid)
  //数据操作接口
  StorageResponse dataOperate(1:StorageRequest request, 2:string uuid)
  //检索接口
  QueryResponse dataQuery(1:QueryRequest request, 2:string uuid)
  //批量检索接口
  map<string,QueryResponse> dataBatchQuery(1:list<QueryRequest> queryRequests, 2:string uuid)
  //检测server心跳
  PingResponse ping()
}